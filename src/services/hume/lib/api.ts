...
export async function fetchChatMessages(humeChatId:string){
  ...
  const chatEventsItreator = await client.empathicVoice.chats.listChatEvents(humeChatId , {pageNumber:0 , pageSize:100})
  for await(const chatEnvent of chatEventsItreator){
    allchatEvents.push(chatEnvent)
  }

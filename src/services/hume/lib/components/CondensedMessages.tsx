...
export  function CondensedMessages({messages , user , className , maxFft = 0}:{
  ...
  return <div className={cn("flex flex-col gap-4 w-full" , className)}>
    {messages.map((message , index)=>{
      ...
      return <div key={index} className={cn("flex items-center gap-6 border pl-4 pr-6 py-4 rounded max-w-3/4" , message.isUser ? "self-end" :"self-start")}>
        {message.isUser ? (
          <UserAvatar user={user} className="size-6 flex-shrink-0"/>
        ): (<div className="relative">
          <div className={cn("absolute inset-0 border-muted border-4 rounded-full" , 
            shouldAnimate ? "animate-ping" :"hidden"
          )}/>
          <BrainCircuitIcon className="size-6 flex-shrink-0" style={shouldAnimate ? {scale:maxFft/8+1} : undefined}/>
          </div>)}
